...
const toCamelCase = <T extends string>(str: T) =>
  str.replace(/-([a-z])/g, (match) => match.replace('-', '').toUpperCase()) as ToCamelCase<T>;

const toKebabCase = <T extends string>(str: T) => str as ToKebabCase<T>;

const toPascalCase = <T extends string>(str: T) =>
  capitalize(str).replace(/-([a-z])/g, (match) => match.replace('-', '').toUpperCase()) as ToPascalCase<T>;

const toSnakeCase = <T extends string>(str: T) => str.replaceAll('-', '_') as ToSnakeCase<T>;
